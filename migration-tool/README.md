...
```bash
wget https://opengauss.obs.cn-south-1.myhuaweicloud.com/latest/tools/openGauss-FullReplicate-7.0.0rc3.tar.gz
```
...
openGauss-FullReplicate/openGauss-FullReplicate-7.0.0rc3.jar
```

其中openGauss-FullReplicate-7.0.0rc3.jar为工具的主程序，config文件夹下为配置文件模板。
...
# 迁移约束
java -jar openGauss-FullReplicate-7.0.0rc3.jar --start constraint --source sqlserver --config /**/**/config.yml
