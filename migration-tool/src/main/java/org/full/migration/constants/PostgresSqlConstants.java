...
/**
 * PostgresSqlConstants
 *
 * @since 2025-05-12
 */
public final class PostgresSqlConstants {
    /**
    ...
    /**
     * sql for querying patition table size
     */
    public static final String QUERY_PATITION_TABLE_SIZE_SQL = "WITH PartitionStats AS (\n" +
            "    SELECT\n" +
            "        parent.relname AS parent_table,\n" +
            "        child.relname AS partition_name,\n" +
            "        pg_stat_user_tables.n_live_tup AS partition_rows,\n" +
            "        pg_table_size(child.oid) AS partition_size,\n" +
            "        CASE \n" +
            "            WHEN pg_stat_user_tables.n_live_tup = 0 THEN NULL\n" +
            ...
            "    WHERE\n" +
            "        ns.nspname = '%s' AND parent.relname = '%s'\n" +
            ")\n" +
            "SELECT\n" +
    ...
    public static final String QUERY_GENERATE_DEFINE_SQL = "SELECT\n"
        ...
        + "    CASE\n"
        + "        WHEN a.attgenerated = 's' THEN TRUE\n"
        + "        ELSE FALSE\n"
        + "    END AS is_stored\n"
        + "FROM\n"
     ...
     */
    public static final String GET_XLOG_LOCATION_NEW = "select pg_current_wal_lsn() AS max_lsn";
     ...
     */
    public static final String MAX_LSN_SQL = "SELECT pg_current_wal_lsn() AS max_lsn";
     ...
     */
    public static final String QUERY_SEQUENCE_SQL = "SELECT\n"
        + "    c.relname AS name,\n"
        ...
        + "    pg_sequence_last_value(c.oid::regclass) AS currentValue,\n"
        + "    (SELECT oid FROM pg_type WHERE typname = 'int8') AS typeId  -- 默认假设为bigint类型\n"
        + "FROM\n"
        + "    pg_sequence s\n"
        + "JOIN\n"
     ...
     */
    public static final String PG_DROP_LOGICAL_REPLICATION_SLOT = "SELECT pg_drop_replication_slot('%s');";
