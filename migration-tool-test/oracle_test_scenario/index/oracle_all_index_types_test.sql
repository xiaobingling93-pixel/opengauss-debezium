...
-- 1.2 分区表（用于演示分区索引）
DROP TABLE IF EXISTS t_part PURGE;
CREATE TABLE t_part (
    id          NUMBER,
    code        VARCHAR2(10),
    status      VARCHAR2(10),
...
DROP TABLE IF EXISTS t_iot PURGE;
CREATE TABLE t_iot (
...
DROP TABLE IF EXISTS t_text PURGE;
CREATE TABLE t_text (
    id      NUMBER PRIMARY KEY,
...
DROP TABLE IF EXISTS t_dim PURGE;
DROP TABLE IF EXISTS t_fact PURGE;
CREATE TABLE t_dim (
...
INSERT INTO t_part (id, code, status, create_date, amount)
SELECT LEVEL,
       'CODE' || MOD(LEVEL, 100),
       CASE MOD(LEVEL, 5) 
           WHEN 0 THEN 'ACTIVE' 
...
CREATE UNIQUE INDEX idx_unique_code ON t_normal(code) WHERE code IS NOT NULL;  -- 部分唯一索引
...
CREATE INDEX idx_reverse ON t_normal(id) REVERSE;

-- 3.5 分区索引
-- 本地分区索引（与表分区对齐）
CREATE INDEX idx_local_part ON t_part(create_date) LOCAL;
-- 全局非分区索引（普通索引，不分区）
CREATE INDEX idx_global_nopart ON t_part(code) GLOBAL;
-- 全局分区索引（按 amount 列分区）
CREATE INDEX idx_global_part ON t_part(amount) GLOBAL
...
WHERE index_name IN ('IDX_LOCAL_PART', 'IDX_GLOBAL_PART')
...
-- SELECT * FROM t_part WHERE create_date >= DATE '2023-06-01';
...
DROP INDEX idx_local_part;
DROP INDEX idx_global_nopart;
DROP INDEX idx_global_part;
DROP INDEX idx_domain_text;
DROP INDEX idx_virtual;
DROP INDEX idx_invisible;
DROP INDEX idx_bitmap_join;
DROP INDEX idx_iot_secondary;

-- 删除表
DROP TABLE t_normal PURGE;
DROP TABLE t_part PURGE;
